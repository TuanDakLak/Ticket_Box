...
import { createConcert, updateConcert, getConcertById } from "@/services/concert.service";
import { ChevronRight, Info, ImagePlus, Bot, Upload, Sparkles, Ticket, PlusCircle, Trash2 } from "lucide-react";
...
function EventForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    poster_url: "https://lh3.googleusercontent.com/aida-public/AB6AXuA96Q00R_bgOVwdSaXoQUFh4qVfI9j-ywdZH0M0n3UEcHkvg27Hc-IVfeqDv0zY5rITz7LfLg-PsHR9fs9vCYLfdTAr48gFSFvlNJyw4aYMTmFgn4tN5xZElV5qJh_mOyC71TmCRwrv-jb1WAzhPD1I6c0R12LHOwt6JrVxYEjLIbk9nj2yHFMRzZzrZ2Vw_pevGqUI5SmxPE1-MUNxiSPVF38B0OBBXFGSoYc6d9xUgDg0Ex-TwrOwqrqg3paEsKJJvwFVtnwg9sih",
    status: "PUBLISHED"
  });

  const [ticketCategories, setTicketCategories] = useState<TicketCategory[]>([
    { name: "General Admission", price: 500000, total_quantity: 1000, max_per_user: 4 }
  ]);
  ...
  useEffect(() => {
    if (editId) {
      getConcertById(editId).then((data) => {
        ...
        setFormData({
          ...
          location: data.venue && data.city ? `${data.venue}, ${data.city}` : data.venue || "",
          ai_bio: data.aiBio || "",
          start_time: formattedDate,
          svg_map_url: data.mapUrl || "https://cdn.ticketbox.local/maps/default.svg",
          poster_url: "https://lh3.googleusercontent.com/aida-public/AB6AXuA96Q00R_bgOVwdSaXoQUFh4qVfI9j-ywdZH0M0n3UEcHkvg27Hc-IVfeqDv0zY5rITz7LfLg-PsHR9fs9vCYLfdTAr48gFSFvlNJyw4aYMTmFgn4tN5xZElV5qJh_mOyC71TmCRwrv-jb1WAzhPD1I6c0R12LHOwt6JrVxYEjLIbk9nj2yHFMRzZzrZ2Vw_pevGqUI5SmxPE1-MUNxiSPVF38B0OBBXFGSoYc6d9xUgDg0Ex-TwrOwqrqg3paEsKJJvwFVtnwg9sih",
          status: data.status || "PUBLISHED"
        });

        if (data.ticketTiers && data.ticketTiers.length > 0) {
          setTicketCategories(data.ticketTiers.map(t => ({
            id: t.id,
            name: t.name,
            price: t.price,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`Failed to save event: ${error.response?.data?.message || "Please check the inputs."}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleTierChange = (index: number, field: keyof TicketCategory, value: string | number) => {
  ...
  return (
    <div className="space-y-8 pb-20 md:pb-0">
      ...
      <header className="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4 pb-4 border-b border-border">
        <div>
          <nav aria-label="Breadcrumb" className="flex text-muted-foreground font-body text-xs font-semibold mb-2">
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
              <li className="inline-flex items-center">
                <Link className="hover:text-primary transition-colors" href="/admin/events">Events</Link>
              </li>
              <li>
                <div className="flex items-center">
                  <ChevronRight className="w-4 h-4 mx-1" />
                  <span className="text-foreground">{isEditing ? "Edit Event" : "Create New Event"}</span>
                </div>
              </li>
            </ol>
          </nav>
        ...
        <div className="space-y-8">
          ...
          <section className="bg-surface rounded-xl p-6 shadow-sm border border-border">
            ...
            <div className="space-y-5">
              <div>
                <label className="block font-body text-xs font-semibold text-foreground mb-1">Event Name</label>
                <input 
              ...
              <div>
                <label className="block font-body text-xs font-semibold text-foreground mb-1">Description</label>
                <input 
                  className="w-full rounded-lg border border-border bg-background px-4 py-3 text-foreground focus:border-primary focus:ring-1 focus:ring-primary transition-shadow text-sm" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block font-body text-xs font-semibold text-foreground mb-1">Venue Location</label>
                  <input 
                ...
                <div>
                  <label className="block font-body text-xs font-semibold text-foreground mb-1">Date & Time</label>
                  <input 
                    className="w-full rounded-lg border border-border bg-background px-4 py-3 text-foreground focus:border-primary focus:ring-1 focus:ring-primary text-sm" 
                    type="datetime-local"
              ...
              <div>
                <label className="block font-body text-xs font-semibold text-foreground mb-1">Cover Image <span className="text-muted-foreground font-normal">(Optional)</span></label>
          ...
          <section className="bg-surface rounded-xl p-6 shadow-sm border border-border">
            ...
            <div className="p-4 rounded-lg bg-surface-low border border-border mb-5">
              <h4 className="font-body text-xs font-semibold text-foreground mb-2">Upload Press Kit (PDF) <span className="text-muted-foreground font-normal">(Optional)</span></h4>
              <div className="flex items-center gap-4">
                <button className="px-4 py-2 rounded-lg border border-border text-foreground hover:bg-surface-high transition-colors text-sm font-medium flex items-center gap-2">
            ...
            <div className="relative">
              <label className="block font-body text-xs font-semibold text-foreground mb-1 flex justify-between">
                <div>Event Description (AI Generated) <span className="text-muted-foreground font-normal">(Optional)</span></div>
                <button 
              ...
              <textarea 
                ...
                onChange={e => setFormData({...formData, ai_bio: e.target.value})}
                disabled={isGeneratingBio}
              ></textarea>
            </div>
          </section>
          ...
          <section className="bg-surface rounded-xl p-6 shadow-sm border border-border">
            <div className="flex justify-between items-center border-b border-border pb-4 mb-6">
              ...
              <button onClick={handleAddTier} className="text-primary font-body text-xs font-semibold flex items-center gap-1 hover:underline">
                <PlusCircle className="w-4 h-4" /> Add Tier
              </button>
            </div>

            {ticketCategories.map((tier, index) => (
              <div key={index} className="border border-border rounded-xl p-5 mb-4 bg-background relative overflow-hidden group">
                <div className={`absolute top-0 left-0 w-1 h-full ${index % 2 === 0 ? 'bg-secondary' : 'bg-primary'}`}></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pl-2">
                  <div>
                    <label className="block font-body text-xs font-semibold text-muted-foreground mb-1">Price (VND)</label>
                    <input 
                  ...
                  <div>
                    <label className="block font-body text-xs font-semibold text-muted-foreground mb-1">Total Qty</label>
                    <input 
                      className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-foreground focus:border-primary focus:ring-1 focus:ring-primary text-sm" 
                      type="number" 
                      value={tier.total_quantity}
                      onChange={(e) => handleTierChange(index, "total_quantity", Number(e.target.value))}
                    />
                  </div>
                  <div>
                    <label className="block font-body text-xs font-semibold text-muted-foreground mb-1">Max/User</label>
                    <input 
                      className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-foreground focus:border-primary focus:ring-1 focus:ring-primary text-sm" 
                      type="number" 
                      value={tier.max_per_user}
                      onChange={(e) => handleTierChange(index, "max_per_user", Number(e.target.value))}
                    />
                  </div>
                </div>
              </div>
